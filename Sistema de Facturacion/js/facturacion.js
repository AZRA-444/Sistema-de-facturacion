...
function actualizarTabla() {
  ...
  // CORRECCIÓN: Guardado correcto en el objeto de estado global
...
// CORRECCIÓN: Delegación de eventos para evitar usar 'onclick' en strings HTML
...
async function finalizarCompra() {
  ...
  // Desactivar botón y activar su animación local (si la tiene)
  boton.disabled = true;
  boton.classList.add("animating");

  // DISPARAR EL MODAL: Muestra el spinner de carga inmediatamente
  ...
  try {
    ...
    // CAMBIO DE ESTADO: Oculta spinner y muestra el Check verde de Éxito
    mostrarModalExito();
    
    // Autorizar la recarga de página (por tu evento beforeunload)
    state.compraExitosa = true; 
    state.listaProductos = [];
    actualizarTabla();
    ocultarSeccionPagos();

    // LE DAMOS TIEMPO AL USUARIO: Esperamos 2 segundos para que disfrute el "¡Listo!"
    setTimeout(() => {
      location.reload(); 
    }, 2000);
    ...
    // CAMBIO DE ESTADO: Oculta spinner y muestra la cruz roja con el mensaje real
